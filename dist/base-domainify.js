'use strict';
var BaseDomainify, Facade, MasterDataResource, Path, coffee, fs, requireFile, through,
...
BaseDomainify = (function() {
  ...
  BaseDomainify.prototype.getInitialCode = function(dirname, modulePaths) {
    ...
    _ = ' ';
    coffeeCode = "__ = (m) -> if m.default then m.default else m\nFacade = __ require '" + this.moduleName + "'\n\nFacade::init = ->\n" + _ + "return unless @dirname.match '" + basename + "'\n";
  ...
  BaseDomainify.prototype.getScriptToLoadCoreModule = function(dirname) {
    ...
    for (i = 0, len = ref.length; i < len; i++) {
      ...
      coffeeCode += _ + "@addClass '" + name + "', __ require('" + path + "')\n";
    }
    return coffeeCode;
  };
  ...
  BaseDomainify.prototype.getScriptToLoadModule = function(moduleName, moduleDirname) {
    ...
    for (i = 0, len = ref.length; i < len; i++) {
      ...
      coffeeCode += "" + _ + _ + "@addClass '" + moduleName + "/" + name + "', __ require('" + path + "')\n";
    }
    return coffeeCode;
  };
  ...
  BaseDomainify.prototype.getMasterJSONPath = function(dirname) {
    var e, error, facade, masterJSONPath, relPath;
    try {
