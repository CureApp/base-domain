...
BaseModel = (function(superClass) {
  ...
  BaseModel.prototype.toPlainObject = function() {
    ...
    for (prop in this) {
      ...
      value = this[prop];
      if (modelProps.isEntity(prop) || modelProps.checkOmit(prop)) {
        continue;
      }
  ...
  BaseModel.prototype.included = function(recursive) {
    var entityProp, i, idPropName, j, len, len1, modelProp, modelProps, ref, ref1;
    if (recursive == null) {
      recursive = false;
    }
    modelProps = this.getModelProps();
    ...
    for (i = 0, len = ref.length; i < len; i++) {
      entityProp = ref[i];
      idPropName = modelProps.getTypeInfo(entityProp).idPropName;
      if ((this[idPropName] != null) && (this[entityProp] == null)) {
        return false;
      }
    }
    if (!recursive) {
